...
use soroban_debugger::debugger::{DebugState, InstructionPointer, Stepper};
use soroban_debugger::runtime::{Instruction, InstructionParser};
use soroban_debugger::ui::formatter;
...
fn test_stepper_without_executor() {
    ...
    use soroban_debugger::debugger::{Stepper, DebugState};
...
fn test_stepping_pause_conditions() {
    ...
    let simple_inst = Instruction::new(
        0x100,
...
fn test_instruction_pointer_state_formatting() {
    ...
    let formatted = Formatter::format_instruction_pointer_state(
        42,
        3,
        Some(StepMode::StepInto),
        true,
    );
...
fn test_debug_state_instruction_management() {
    ...
    let instructions = vec![
        ...
        Instruction::new(0x105, wasmparser::Operator::LocalSet { local_index: 0 }, 0, 1),
        Instruction::new(0x107, wasmparser::Operator::LocalGet { local_index: 0 }, 0, 2),
    ];
...
fn test_instruction_parsing_performance() {
    ...
    assert!(elapsed.as_millis() < 1000, "Instruction parsing too slow: {:?}", elapsed);
}